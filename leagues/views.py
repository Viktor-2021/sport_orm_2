...
def index2(request):
    context = {
        ...
        # 2
        "players_boston_penguins": Player.objects.filter(curr_team__location="Boston", curr_team__team_name=""),
        # 3
        ...
        #14
        "all_teams_whit_12_players":Team.objects.annotate(num_players=Count('all_players')).filter(num_players__gt=12),
        #15
        "all_player_in_teams": Player.objects.annotate(num_teams=Count('all_teams')).order_by('-num_teams')
